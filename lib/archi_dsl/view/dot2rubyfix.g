...
BEGIN {
  ...
  graph_t sub_graph;
  graph_t tmp_g;
  graph_t check_parent_graph;
  graph_t tmp_check_parent;
  ...
  void write_graph(graph_t sub_graph) {
  ...
  graph_t highest_unseen_node(graph_t child) {
    if (child == NULL) {
      return NULL;
    }
    if (seen_sub[child.name] == 1) {
      return NULL;
    }
    tmp_check_parent = highest_unseen_node(child.parent);
    if (tmp_check_parent == NULL) {
      if (seen_sub[child.name] != 1) {
        return child;
      }
    }
    return tmp_check_parent;
  }
}
...
N {
  ...
  check_parent_graph = highest_unseen_node(ofgraph);
  if ( check_parent_graph != NULL ) {
    write_graph(check_parent_graph);
    load_sub_graph(check_parent_graph);
  }
